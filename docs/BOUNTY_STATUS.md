...
4. **ENS — Best Use of ENSv2, $4,500**

The project has strong technical evidence for these four prizes. Final eligibility still depends on the ETHGlobal submission, demo recording, public deployment checks, and the organizers' start-fresh determination.

The current workspace contains the durable report snapshot, corrected Graph accounting and Sepolia RPC update. Deploy these changes before using the public URLs in the final recording.

The visible repository history starts on September 5, 2026, after ETHOnline began on September 4. Because development also exists in a collaborator repository, disclose that history and let ETHGlobal determine **Start Fresh** eligibility; the date alone is not conclusive proof.

## Readiness matrix

| Prize | Verdict | Evidence already complete | Missing qualification or proof |
|---|---|---|---|
| The Graph — AI Tooling / AI Use Case, $5,000 | **Strong technical fit; proof pending** | The Graph is load-bearing; live Gateway data drives ranking, uncertainty decisions and natural-language output. The Streamable HTTP MCP tool now persists a completed no-purchase session and has a `SKILL.md`. | Deploy, confirm public MCP initialize/tools/call, document project provenance, then submit the required 2–4 minute video. |
| The Graph — Composable / Standardized Products, $5,000 | **Technical qualification met** | One Messari Lending/CDP query template runs across five Base Messari deployments. The live run labels the separate Aave native adapter independently and explains the leverage of the shared schema. | Record and submit the required 2–4 minute video. Put the five deployment IDs and one shared query in the README proof section. |
| Privy — Best financial flow, $2,500 | **Strong technical fit; fresh demo pending** | A real Privy server wallet, attached signing policy and SDK signer completed a $0.03 Base Sepolia USDC x402 payment. New sessions require Privy authentication; ownership, serialized authorization and an aggregate recorded-settlement cap protect the shared treasury. | Configure persistent production storage, run a fresh authenticated payment, record it and explain how Privy removes seed/key handling. |
...
| ENS — Best Use of ENSv2, $4,500 | **Strong technical fit; fresh demo pending** | scout-agent.eth is registered on ENSv2 Sepolia with a Permissioned Resolver. The EAC endpoint now calls a result verified only when an unauthorized transaction is mined and reverts; RPC failures remain indeterminate. The ENS budget is checked with the aggregate recorded-settlement cap before payment. | Redeploy and record the authenticated identity/permission/payment flow. |
...
- Preserved paid run: 11 candidates, 15 evidence sources, $0.03 USDC spent, sFRAX on Compound V3 selected with a 51.4 opportunity score and 30 risk score. The run disclosed two skipped Graph deployments and sparse OpenSEO results.
...
1. Deploy the current `dev` branch, restart the API once, and verify the bundled report snapshot survives the restart.
2. Confirm the public agent page resolves `scout-agent.eth` and the configured Sepolia RPC can broadcast the scoped status write.
3. Record one 2–4 minute video: prompt → standardized Graph deployments → ranked candidates → uncertainty gate → Privy approval → onchain payment → ENS identity/MCP record → paid result.
4. Submit to both Graph prizes, Privy Best financial flow and ENS Best Use of ENSv2.
5. Complete Bazantic only after the primary submissions are ready.
...
- [Namesake](https://ethglobal.com/showcase/namesake-kq1ez) made ENS provisioning central through a real subname, records and an agent wallet. Scout's current proof similarly uses a registered name and scoped agent wallet, while also reading the budget and MCP records in its runtime path.
