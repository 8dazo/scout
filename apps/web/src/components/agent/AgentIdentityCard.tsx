...
export function AgentIdentityCard({
  ...
  const configured = Boolean(ensName);
    ...
    <div className="border-brutal p-8 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className={`font-mono text-sm mt-1 ${configured ? "text-signal" : "text-ink/50"}`}>
            ENS: {ensName ?? "ENS unconfigured"}
          </p>
          {mcpEndpoint && (
            <a
              href={mcpEndpoint}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 font-mono text-sm pt-4 border-t border-ink/10">
        <div>
          ...
          <p className={`font-bold ${configured ? "text-success" : "text-ink/50"}`}>{status}</p>
        </div>
        <div>
          <p className="text-ink/50 uppercase text-xs">Budget Cap</p>
          <p className="font-bold">
            {typeof budgetCap === "number" ? `$${budgetCap.toFixed(2)} USDC` : "—"}
          </p>
        </div>
        <div>
          <p className="text-ink/50 uppercase text-xs">Role Model</p>
          <p className="text-ink/80">{configured ? "EAC Permissioned" : "Unconfigured"}</p>
        </div>
        <div>
          <p className="text-ink/50 uppercase text-xs">Integrations</p>
          <div className="flex flex-wrap gap-1 mt-1">
            <ToolBadge tool="graph" />
            <ToolBadge tool="ens" />
            <ToolBadge tool="privy" />
          </div>
        </div>
      </div>
    </div>
